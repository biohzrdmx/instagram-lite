...
;(function($) {
	...
	$.fn.instagramLite = function(options) {
		...
		var loadContent = function() {
			...
			if(s.clientID && (s.username || s.hashtag)) {

				var fetchMedia = function(thisUser) {
					// construct API endpoint
					if (s.username && !s.hashtag) {
						var url = 'https://api.instagram.com/v1/users/'+thisUser.id+'/media/recent/?client_id='+s.clientID+'&count='+s.limit+'&callback=?';
					} else if (s.hashtag) {
						var url = 'https://api.instagram.com/v1/tags/'+s.hashtag+'/media/recent/?client_id='+s.clientID+'&count='+s.limit+'&callback=?';
					}
